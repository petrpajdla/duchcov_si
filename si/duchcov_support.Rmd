...
> Claiming of land or a production deposit? The Duchcov hoard in Bohemia as a proxy for "Celtic migrations" in Europe in the 4 th century BC

`R` version 3.6.3 was used [@rct2020] with the following packages:
...
Most of the data is enclosed as comma separated values (CSV) files.
The only source that is not shared is the data consisting of lead isotope signals
for different ore sources because it was collected from various sources including
not published material.

Plots and/or tables included in the text of the article are not shown here.
...
```{r kmeans}
# ------------------------------------------------------------------------------
# please note: to avoid confusion in changes in k-means cluster numbers/colors/shapes
...
# write_csv(cluster, here::here("data", "clusters.csv")))
...
## Data distribution in isospaces
...
```{r duchcov-prediction-tab, fig.cap="Posterior probabilities for predicted sources"}
...
DT::datatable(duchcov_prediction_df,
              caption = "Posterior probabilities for predicted sources",
              rownames = FALSE,
...
duchcov_prediction_df %>%
  ...
  group_by(kmeans, region) %>%
  dplyr::summarize(mean_prob = mean(probability, na.rm = TRUE)) %>% 
  tidyr::spread(key = kmeans, value = mean_prob) %>% 
  mutate(across(where(is.numeric), round, 3)) %>% 
  write_excel_csv(here::here("data", "mean_probability.csv"))
```
-->
...
knitr::kable(addmargins(xtabs(~kmeans + fct_drop(predicted_region),
                              duchcov_prediction_df)),
             caption = "Numbers of data points from different K-means clusters per different sources ")
```
